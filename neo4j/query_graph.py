...
def query_num_connections_by_date(tx, company1, company2, schema, date):
    ...
    return tx.run(query, years = date[2], days = date[1], months = date[0], company1 = company1, company2 = company2).single().value()
...
def get_num_connections(company1, company2, schema = ['name', 'name'], date = None):
    ...
    # date is None by defult (no date limit when querying), but if specified it is assumed to be in the format [month, date, year] where each element in the 
    # list is a positive integer specifying how far back to look for relationships (subject to change)
...
def query_connection_list_by_date(tx, company, attribute, date):
    ...
    return tx.run(query, years = date[2], days = date[1], months = date[0], company = company).single().value()
